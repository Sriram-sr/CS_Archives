...
def enable_hotspot():
    try:
        ...
        if read_from_system_file(dhcpcd_conf_file_data,"/etc/dhcpcd.conf") == FAILURE:
        ...
        if (splitted_dhcpcd_data[-1] != DENY_INTERFACES):
            splitted_dhcpcd_data.append(DENY_INTERFACES)
        ...
        if write_to_system_file(new_dhcpcd_data, "/etc/dhcpcd.conf",
            ...
            return FAILURE
        if write_to_system_file(ALLOW_INTERFACE, "/etc/network/interfaces",
                                                            'w') == FAILURE:
            print("Error occured while writing to interface conf file")
            return FAILURE
        if write_to_system_file(HOSTAPD_ENABLE, "/etc/default/hostapd",
                                                            'w') == FAILURE:
            print("Error occured while writing to hostapd conf file")
            return FAILURE
        cmd_output = {}
        if command_executor(cmd_output, ENABLE_CMD) != SUCCESS:
            print("Command Output : ", cmd_output)
            return FAILURE
        print("Successfuy enabled")
        return SUCCESS
    except Exception as error:
        print("error in enable hotspot : {} ".format(error))
        return FAILURE
enable_hotspot()
